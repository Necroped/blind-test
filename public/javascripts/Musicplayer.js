...
Musicplayer.updateDOM = function() {
  ...
  if (prevTrack) {
    $('#trackname_prev').html(
      'Previous : ' + prevTrack.title + ' - ' + prevTrack.artist.name
    );
  }
  $('#trackname').html(
    'Current : ' + currentTrack.title + ' - ' + currentTrack.artist.name
  );
  if (nextTrack) {
    $('#trackname_next').html(
      'Next : ' + nextTrack.title + ' - ' + nextTrack.artist.name
    );
  }
};
...
Musicplayer.getPrevTrack = function() {
  return DZ.player.getCurrentIndex() > 0
    ? DZ.player.getTrackList()[DZ.player.getCurrentIndex() - 1]
    : false;
};
...
Musicplayer.getNextTrack = function() {
  return DZ.player.getCurrentIndex() < DZ.player.getTrackList().length - 1
    ? DZ.player.getTrackList()[DZ.player.getCurrentIndex() + 1]
    : false;
};
